...
catch {set coef_files [glob -nocomplain am_fir_coef*.txt]} error_msg
puts "$error_msg"
  
# The top-level in HDL type "VERILOG"
set ipfs_ext vo
if {[file exists am_fir.vo]} {
	set hdl_ext v
} else {
	puts "Warning: Could not find am_fir.$ipfs_ext!"
}
...
set_global_assignment -name EDA_TEST_BENCH_MODULE_NAME work.tb_am_fir -section_id tb
set_global_assignment -name EDA_TEST_BENCH_GATE_LEVEL_NETLIST_LIBRARY work -section_id tb

# IPFS file
set_global_assignment -name EDA_IPFS_FILE am_fir.$ipfs_ext -section_id eda_simulation -library work
...
set_global_assignment -name EDA_TEST_BENCH_FILE am_fir_input.txt -section_id tb -library work

set_global_assignment -name EDA_TEST_BENCH_FILE tb_am_fir.vhd -section_id tb -library work
